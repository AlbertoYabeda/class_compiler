package constantes;

import modelos.Identificadores;

public class Tipos {
    public final String INICIO_ESCOPO = "simbolo de inicio de escopo";
    public final String FIM_ESCOPO = "simbolo de fim de escopo";
    public final String DELIMITADOR_INICIO_PARAMETRO = "simbolo delimitador de inicio de parametros";
    public final String DELIMITADOR_FIM_PARAMETRO = "simbolo delimitador de fim de parametros";
    public final String FIM_DE_INSTRUCAO = "Simbolo de fim da instrucao";
    public final String PONTO = "Simbolo de acesso a objecto";
    public final String VIRGULA = "separador de elementos de lista";
    public final String ATRIBUICAO = "constante de atribuicao";
    public final String CONSTANTE_NUM = "constante numerica";

    public final String CLASS_WORD = "class";
    public final String PRIVATE = "private";
    public final String PUBLIC = "public";
    public final String INT = "int";
    public final String LONG = "long";

    public Tipos(){}
}
